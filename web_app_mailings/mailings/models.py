...
class Mailing(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.start_time and timezone.is_naive(self.start_time):
            self.start_time = timezone.make_aware(self.start_time, moscow_tz)
        if self.end_time and timezone.is_naive(self.end_time):
            self.end_time = timezone.make_aware(self.end_time, moscow_tz)
...
class Attempt(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=(("Успешно", "Успешно"), ("Не успешно", "Не успешно")), verbose_name="Статус"
    )
